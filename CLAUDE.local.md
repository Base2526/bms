...
- **`9.54__bms_kitchen_station_master.sql` (สถานีครัวเป็น entity: ทะเบียน + ผูกสาขา + เปิด/ปิด +
  ...
  **pure 724 เทสผ่านทั้งหมด** · production build ผ่าน · **⚠️ ยังไม่ได้ apply เข้า dev DB และยังไม่ได้
  ...
  · **⚠️ createOrder / sendRestaurantKitchenRound / readiness / หน้าสั่งอาหาร อ่าน
    `bms_kitchen_stations` (LEFT JOIN) กับทุกบิลของทุกร้าน** — ต้อง apply migration **ก่อน** deploy
    โค้ดชุดนี้เสมอ (กฎเดียวกับ `9.40`/`9.51`)
  ...
  · **นิพจน์เลือกสถานีมีชุดเดียว** `kitchenStationColumnsSql()` — ผู้เรียก 3 ที่ (คิวครัวบิลค้าปลีก ·
    รอบครัวบิลโต๊ะ · การอ่านรายตัว) · สองสูตรจะ drift แล้ววันหนึ่งเส้นทางหนึ่งยอมส่งตั๋วข้ามสาขา
  ...
  · **ชื่อที่มาทางเส้นทางเก่าถูกยกขึ้นเป็นแถวหลักอัตโนมัติ** (`ensureKitchenStationByNameInTx`) ที่
    ฟอร์มสินค้า/ไฟล์นำเข้า/`devSeed` — ไม่ทำแบบนี้ = สถานีกำพร้างอกขึ้นเรื่อย ๆ ซึ่งเปิด/ปิดไม่ได้
    เรียงไม่ได้ และตั้งเกณฑ์เวลาให้ได้ต่อเมื่อพิมพ์ชื่อตรงเป๊ะ (= อาการที่งานนี้ทำมาเพื่อเลิก)
  ...
  · เทส: `scripts/kitchen-station-contract.test.mts` (14 เทส pure) + 5 เทสใหม่ใน
    `kitchen-board-contract` (รวม 21) + `scripts/kitchen-station-db-contract.test.mts` (18 เทส DB
