...
## recheck POS ร้านอาหารทั้งเส้น — เจอของจริง 1 จุดในเส้นเงิน (2026-09-08)

branch `recheck/pos-restaurant-2026-09-08` (ตัดจาก `develop` ที่ merge #192 แล้ว) ·
`npm run gate` ผ่าน (typecheck · **pure 933/933** จาก 932 · production build) ·
...
`restaurant-pos-contract` **57 เทส** (จาก 56) — ตัวใหม่เดินทั้งสองฟังก์ชันแล้วบังคับสามข้อ:
...
### ⚠️ ข้อสังเกตที่ยังไม่แก้ (ต้องมีฐานข้อมูลก่อนถึงจะกล้าแตะ)

- **ลำดับล็อกกลับหัวระหว่าง "ครัวยกเลิกจาน" กับ "ยกเลิกทั้งบิล" → deadlock `40P01`**
  · `updateKitchenTicketStatusInTx` ล็อกแถวตั๋ว (`FOR UPDATE OF rt`) **ก่อน** แล้วค่อยขอ
  กะ + advisory lock ของบิล (ผ่าน `dropKitchenCancelledLineInTx`) · ส่วน `cancelRestaurantCheck`
  ขอ advisory lock ของบิลก่อน แล้วค่อย `UPDATE bms_restaurant_kitchen_tickets` ของทั้งบิล
  → A ถือ rt รอ advisory · B ถือ advisory รอ rt = วน
  · ผลคือ 500 "เซิร์ฟเวอร์ผิดพลาด" ที่จอครัวหรือเครื่องขาย (rollback สะอาด กดใหม่ผ่าน)
  ไม่ใช่เงินหรือสต็อกเพี้ยน · **จงใจไม่แก้ในรอบนี้**: ทางแก้ที่ถูกคือย้ายไปขอ กะ → advisory(บิล)
  → rt ให้ตรงกับอีกสองเส้น ซึ่งต้องแตะเส้นทางที่จอครัวทั้งสองจอใช้ร่วมกัน และ
  **ยืนยันด้วยเทส DB ไม่ได้บนเครื่องนี้** · การเดาลำดับล็อกผิดสร้าง deadlock ใบใหม่ได้ง่ายกว่าเดิม
  (ถ้าขอ advisory ก่อนกะ จะไปชนกับ `sendRestaurantKitchenRoundInTx` ที่ขอกะก่อน)
- **`updateKitchenTicketsStatus` ไม่เรียง `ticketIds` ก่อนล็อก** — สองคำขอที่ส่งชุดเดียวกัน
  คนละลำดับจะ deadlock กันเอง · วันนี้จอส่งลำดับที่ server กำหนดมา จึงยังไม่เคยเกิด
- **เพดานของ QR rate limit ผูกกับ "รูปทรง" ของคุกกี้ ไม่ใช่ session ที่ยืนยันแล้ว**
  (`/^[A-Za-z0-9_-]{32,128}$/`) → หมุนคุกกี้มั่ว ๆ ได้ bucket ใหม่ไม่จำกัดสำหรับ scope ที่ไม่ใช่
  `open` · **ไม่ทำให้กล่องขาเข้าของพนักงานถูกถล่ม** เพราะการสร้างคำขอจริงต้องใช้ session token
  ตัวจริงซึ่งคงที่ (bucket เดียว) + เพดาน `MAX_PENDING_SUBMISSIONS_PER_CHECK` · เป็นการเปลือง
  query ของฐานล้วน ๆ · ทางแก้ที่ถูกคือซ้อน bucket ต่อ IP อีกชั้น ไม่ใช่เลิกแยกตาม session
  (ร้านหนึ่งใช้ IP เดียวกันทั้งร้าน ซึ่งเป็นเหตุผลที่โค้ดนี้มีอยู่)
- **`split_group_no` เดินหน้าอย่างเดียว** (`MAX+1` เฉพาะใบที่ยังเปิด) → โต๊ะที่ปิดใบเลขน้อย
  แล้วแยกจากใบเลขมากซ้ำ ๆ จะไต่ถึง 20 แล้วขึ้นว่า "ครบเพดาน 20 ใบ" ทั้งที่เปิดอยู่ใบเดียว ·
  ต้องวนซ้ำ ~20 รอบบนโต๊ะเดียวถึงจะเจอ · ทางแก้คือหาเลขว่างที่น้อยที่สุดแทน MAX+1
- **จอยอมให้เลือกบิลสถานะ `CLOSING` เป็นปลายทางการรวมบิล** แต่ service รับเฉพาะ `OPEN`
  → ได้ error ที่อ่านรู้เรื่อง แต่เป็นสิ่งที่จอรู้ล่วงหน้าได้ (สถานะชั่วคราว เกิดยาก)
