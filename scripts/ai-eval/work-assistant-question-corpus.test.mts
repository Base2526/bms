...
test("the pinned corpus still covers the questions it was written for", () => {
  // 77 questions people actually ask (chips + hand-verified), plus coverage questions for the
  // rest of the catalog, plus 2 guards that must stay unanswerable.
  // (+5 จาก `9.63` แยกบิล/รวมบิล และ +5 จาก `9.64` บัตรคิว/จองโต๊ะ — ทั้งสองชุดเป็นคำที่
  //  ลูกค้าพูดกับพนักงานที่หน้าร้านจริง ๆ ไม่ใช่ชื่อฟีเจอร์)
  assert.equal(CORPUS_REAL_QUESTIONS.length, 77, "a pinned question disappeared or was added without review");
