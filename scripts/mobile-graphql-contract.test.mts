...
test("all 92 mobile/POS input arguments are typed", () => {
  ...
  assert.equal(
    ...
    92,
    "the mobile/POS surface must keep all 92 input-bearing operations",
  );
  assert.deepEqual(
    inputOperations
...
test("all 138 mobile/POS outputs are recursively typed with no JSON escape hatch", () => {
  const operations = moduleOperations();
  assert.equal(
    operations.length,
    138,
