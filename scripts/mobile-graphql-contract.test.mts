...
test("all 90 mobile/POS input arguments are typed", () => {
  ...
  assert.equal(
    ...
    90,
    "the mobile/POS surface must keep all 90 input-bearing operations",
  );
  assert.deepEqual(
    inputOperations
...
test("all 136 mobile/POS outputs are recursively typed with no JSON escape hatch", () => {
  const operations = moduleOperations();
  assert.equal(
    operations.length,
    136,
