...
test("the global sidebar uses bounded feature checks instead of the full capability scan", () => {
  ...
  assert.match(sidebar, /can\('pharmacy\.policy\.read'\)[\s\S]*?admin\/pos-readiness/,
    "POS Readiness must remain available to every authorized POS shop, not only pharmacies");
