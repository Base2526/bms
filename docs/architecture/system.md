...
| AI Tool-Calling (customer + global staff assistant) | ✅ | `lib/bms/tools/{types,runtime,catalog}.ts` · `lib/bms/assistantKnowledge/` · `graphql/bmsAssistant.ts` · global admin Drawer + `/admin/assistant`; POS-only gets deterministic guide search — see [../ai/workflow.md](../ai/workflow.md) and [../ai/tools.md](../ai/tools.md) |
...
| Product promotions (buy-X-get-Y, N-for-a-price) — per branch | ✅ | `lib/bms/{productPromotions,pricing}.ts` · `graphql/bmsProductPromotions.ts` · `8.7` + `9.61__bms_product_promotions_branch_scope.sql` · `/admin/promotions` — `location_id IS NULL` is the store-wide offer and a branch row overrides it for that branch; `pickPromotionForLocation()` is the single rule both the register preview and `createOrder()` call, so the screen and the server cannot price a line differently. Reuses `product.edit`, so no permission seeding — see [../business/pos.md](../business/pos.md#each-branch-runs-its-own-offer-961) |
...
| Credit sales & accounts receivable | ✅ | `lib/bms/{pos,arCredit}.ts` · `9.30__bms_ar_credit_sales.sql` · `/admin/receivables` — `CREDIT` completes the sale and opens an AR invoice in the same settlement transaction rather than counting as drawer cash; limits checked before order creation and again under the account lock; a return reduces debt immediately and an over-collection ledger-transfers to the oldest open invoice — see [../agent-invariants.md](../agent-invariants.md#pos-and-tax) |
...
| Board game cafe operations | ✅ | `lib/bms/{boardGameCafe,boardGameWaitlist,boardGameOffers,boardGamePassRenewals}.ts` · `/admin/board-game` · `/board-game` · `app/api/{bms,pos}/board-game/*` · `app/api/board-game/bookings/*` · `apps/mobile` · migrations `9.79`–`9.83`, `9.89`–`9.94`, `9.96`, `9.98`–`10.4` — timed participants, per-group tabs and one-at-a-time settlement while others keep playing, table moves and merges that leave every bill untouched, member passes (monthly unlimited or hour bundles) with immutable branch scope plus store-credit automatic renewal, server-chosen play-time offers frozen into the charge snapshot, encrypted identity holds for the card kept while a game box is out, a guest service bell that moves nothing but staff attention, a walk-in queue and advance/public reservations with deposits carried by the existing payment ledger, rate snapshots, game-copy loans, atomic POS handoff/receipt lines, opt-in aggregate public discovery, and removable dev fixtures — see [../business/board-game-cafe.md](../business/board-game-cafe.md) |
...
| Windows/Linux POS client | 🧪 MVP shell | `apps/desktop/` — Electron shell for the existing authoritative `/pos` surface, packaged as Windows NSIS plus Linux AppImage/DEB, with first-run server pairing, OS-keystore-backed token storage (Linux fails closed unless Secret Service/KWallet is available), origin-restricted IPC/navigation, and realtime token integration. Release signing, auto-update, direct ESC/POS/LAN printer and drawer support, offline tender, and production hardware verification remain — see [../../apps/desktop/README.md](../../apps/desktop/README.md) |
...
| Reports | ✅ | `lib/bms/{dashboard,reports}.ts` · `9.97__bms_sale_cost_report_foundation.sql` — historical profit reads each line's `cost_amount_snapshot` (`CATALOG_AT_SALE`), so editing a catalog cost can no longer rewrite a closed period; rows reconstructed at migration time are labelled as such — see [../ui/dashboard.md](../ui/dashboard.md) |
| Generated Reports & Document Export | ✅ | `lib/bms/{reportEngine,documentGenerator}.ts` · `7.53__bms_generated_reports.sql` + `9.95` (the persisted `report_type` CHECK now covers every export the engine produces, instead of refusing the newer management reports at insert) · `/admin/reports` AI Report Generator + GraphQL/REST/AI tool entry points — see [../ui/dashboard.md](../ui/dashboard.md) |
...
| Global AI Work Assistant | ✅ | `lib/bms/assistantKnowledge/` · `graphql/bmsAssistant.ts` (`bmsWorkAssistant`) · `components/work-assistant/` — no migration, no new permission. A deterministic bilingual catalog (46 capabilities, 101 guides, no DB/network) grounds a Drawer on every admin page and `/admin/assistant`; `/pos` reuses the catalog as offline guide search with no GraphQL/AI call. Capability status is per capability, not per module (`AVAILABLE`/`CONDITIONAL`/`BETA`/`MOCK`), page context re-ranks but never fabricates a match, and a Confirm button shows the mutation and its server-composed arguments — see [../ai/work-assistant-coverage.md](../ai/work-assistant-coverage.md) |
...
apps/
  ...
  mobile/                Bare React Native POS client (generated GraphQL operations)
  desktop/               Electron POS shell — Windows/Linux, macOS test build; hosts /pos, owns no rule
  ws/                    Subscription-only GraphQL WebSocket gateway — never connects to PostgreSQL
packages/
  realtime/              Event union, audience/permission rules, topics, ticket claims
  graphql-core/          typeDefs/resolvers shared by web and ws
db/
  migrations/*.sql        Idempotent, applied in numeric order
docs/                      This documentation tree
schema.graphql             Committed SDL artifact (`npm run schema:export`)
```
