# บันทึกงานเก่า (local notes archive)

ไฟล์นี้คือเนื้อหาเดิมของ [CLAUDE.local.md](../CLAUDE.local.md) ที่ย้ายออกมา เพื่อให้ไฟล์นั้นเล็กพอที่จะ
โหลดเข้า context ได้ทุก session โดยไม่เปลืองโทเคน — **ไม่ได้ลบ ย้ายมาทั้งดุ้น**

- นี่ไม่ใช่สเปก — สเปกจริงอยู่ที่ [../CLAUDE.md](../CLAUDE.md) → [architecture/](architecture/) ·
  [business/](business/) · [ai/](ai/) · [integrations/](integrations/)
- กฎสำหรับ agent อยู่ที่ [../AGENTS.md](../AGENTS.md) + [agent-invariants.md](agent-invariants.md)
- ประวัติฟีเจอร์ฉบับภาษาอังกฤษอยู่ที่ [feature-log.md](feature-log.md)

**อ่านแบบระวัง**: ทุกหัวข้อคือบันทึก ณ ตอนที่ทำงานนั้นเสร็จ ตัวเลข/ชื่อไฟล์/ข้อความ "ยังไม่ทำ" เป็น
snapshot ไม่ใช่ค่าปัจจุบัน — เช็คกับโค้ดจริงก่อนเชื่อเสมอ
...
> `channel_secret` เดิมใน `bms_tenant_channels`, ไม่มี OAuth/sync worker) — ดู [docs/integrations/lazada.md](integrations/lazada.md)
...
fulfillment address guard, pending improvements) อยู่ที่ [docs/ui/customer360.md](ui/customer360.md)

## Inbox customer tab / merge / reorder / Shopee/Lazada (beta/scaffold)

- **แท็บ "ลูกค้า" + merge + reorder** (`mergeCustomers()`, `reorderFromOrder()`) → รายละเอียดเต็มอยู่ที่ [docs/ui/customer360.md](ui/customer360.md) และ [docs/business/order.md](business/order.md) — คนละอย่างกับ Customer 360 panel ด้านบน (ดู note ในไฟล์นั้น)
- **Shopee/Lazada (beta/scaffold)** → รายละเอียดเต็ม (signature ยังไม่ยืนยัน, channel array กระจายหลายจุด, checklist ก่อน production) อยู่ที่ [docs/integrations/lazada.md](integrations/lazada.md) — **บทเรียนสำคัญที่ยังต้องจำ:** เพิ่ม channel ใหม่ทีไร ต้อง `grep -rn '"line".*"tiktok"' apps/web` ด้วย เพราะมี array enumerate channel กระจายหลายจุด ไม่ได้ derive จาก `Channel` type เดียวกันทั้งหมด (เคยพลาดที่ debug endpoint + fake seeder + playground มาแล้ว)
...
[docs/ai/tools.md](ai/tools.md#registry-only-tool-metadata-disambiguation-not-schema)
...
- **ตั้งใจไม่ทำ**: คืน `redemptions_count` ตอน order ถูก cancel/return (อนุรักษ์ไว้กัน
  ...
  [docs/business/order.md](business/order.md#coupons-discount-codes)), จำกัดโค้ดต่อสินค้า/
...
[docs/integrations/carriers.md](integrations/carriers.md) (adapter/mock/booking) และ
[docs/business/order.md § Shipping](business/order.md#shipping) (customer carrier preference +
...
"Public-page i18n coverage expanded" ใน [docs/feature-log.md](feature-log.md) และ § i18n coverage ใน
[AGENTS.md](../AGENTS.md) สำหรับรายละเอียดครบ (ตอนจบรอบนั้น namespace = 25 ตัว ไม่ใช่ ~12) ·
`/live-dashboard` ตั้งใจไม่แปล (รอข้อมูลจริง) · ยังไม่ได้ apply migration `7.56`/ทดสอบสลับภาษาจริงใน
เบราว์เซอร์ (ค้างจากเดิม)
...
`admin/posts`/`admin/post/**`, `admin/operations-schedule`, `admin/dev/sql-console`) ซึ่งต้องแปล
**เข้า**ไทย ไม่ใช่แปลออก · เลขพวกนี้เป็น snapshot — ให้รัน audit ซ้ำก่อนเชื่อ ไม่ใช่อ่านจากไฟล์นี้:
`grep -rl "useI18n\|resolveBilingual" "apps/web/app/(admin)/admin" --include=*.tsx | wc -l`

## AI usage accounting — แยก "คิดเงิน" / "ยิง provider" / "ต้นทุนจริง" (2026-08-13)

**เสร็จแล้ว (migration `7.82` — ยังไม่ได้ apply เข้า production)** — ต้นเรื่อง: ตัวเลขบิลอธิบายไม่ได้
เพราะเดิมมีแค่คู่เดียว (`credits_used` + `estimated_cost`) รับบทบาทสามอย่างพร้อมกัน → tool loop หลายรอบ,
retry ตอน validate ไม่ผ่าน, หรือ OCR fallback ข้าม provider แยกไม่ออกจาก "ลูกค้าใช้งานเพิ่ม" และหน้า
`/admin/billing` ก็ถมช่องว่างด้วย **ตัวเลขปลอมฝั่ง client** (`estimateUsedCredits()`, `$0.35/credit`
ฮาร์ดโค้ด, `buildMockLedger()` ที่ปั้นแถว grant/usage/top-up ขึ้นมาเอง) — ลบทิ้งหมดแล้ว หน้านี้อ่าน field
จริงจาก `bmsAiUsage`/`bmsAiUsageBreakdown` แทน

- **สามมิติต้องแยกกันตลอด ห้ามยุบกลับ**: `billable_credits` (คิดเงินลูกค้า — 1 credit ต่อ **logical
  request** บนแพลนที่มีลิมิต, **0 เสมอ** บนแพลนไม่จำกัด) · `provider_calls` (จำนวนครั้งที่ยิง provider
  จริง) · `actual_cost_usd` (ต้นทุนที่ attribute จาก token ที่ provider รายงาน × rate card ที่ตั้งไว้)
  · `unpriced_provider_calls` (ยิงไปแล้วแต่ไม่ได้ usage กลับมา = ไม่รู้ต้นทุน)
- **`actual_cost_usd` เป็น NULL ได้ และต้องเป็น NULL เมื่อไม่รู้** — ห้ามเขียน `0` เพราะ `0` อ่านเหมือน
  "ยืนยันแล้วว่าฟรี" · ถ้ายิง 2 ครั้งแต่ได้ usage กลับมาครั้งเดียว ให้เก็บต้นทุนที่รู้ไว้ แล้วนับอีกครั้งเข้า
  `unpriced_provider_calls` · ทุกที่ที่โชว์ยอดต้นทุนต้องโชว์ `unpricedProviderCalls` คู่กันเสมอ
- **`NUMERIC(12,4)` → `NUMERIC(16,8)`** เพราะของเดิมปัดต้นทุนต่อ request ที่เล็กแต่จริงให้เป็น 0 (การ
  backfill จึงตั้งใจ **ไม่** แปลง 0 เก่าเป็นค่าที่ยืนยัน — ปล่อยเป็น NULL ถ้าแยกไม่ออก) · backfill stamp
  `meta.usage_accounting_version = '2'` ไว้ รันซ้ำจึงเป็น no-op
- **retry/fallback ≠ ขายสองครั้ง** — attempt ที่อยู่ใน logical request เดียวกันแชร์
  `meta.usage_group_id` และ `requests` นับ `COUNT(DISTINCT usage_group_id)` → OCR สลิปที่ Qwen ล้ม
  แล้ว fallback ไป Anthropic = **1 credit / 2 provider calls** · ถ้าจะเพิ่ม retry path ใหม่ ให้ reuse
  group id เดิม อย่าสร้าง event ใหม่แล้วนับเป็น request ใหม่
- **`finalizeAiUsageEvent()` เป็น one-shot** (มี cleanup หลายทางที่เรียกซ้อนกันได้จริง) ไม่งั้นต้นทุน
  ก้อนเดียวถูกบวกเข้าสรุปรายเดือนสองรอบ · reservation ที่จบด้วย `provider_calls = 0` จะ **คืน credit
  อัตโนมัติใน transaction เดียวกัน** พร้อมแถว ledger `refund` (`credit_refund_reason:
  provider_not_called`) · `reconcileStaleAiReservations()` กวาด reservation ที่ค้างสถานะ `started`
  เกิน `STALE_AI_RESERVATION_MINUTES = 15` (process ตายกลางทาง) ด้วยกติกาเดียวกัน — เรียกทั้งตอนอ่าน
  `getAiUsage()` และก่อน `tryConsumeAiQuota()`
- **BYOK key test เป็น inference call จริง** จึงบันทึกเป็น event `ai_key_test` แบบ 0 credit (ไม่ใช่ข้าม
  ไปเฉย ๆ) · ส่วน health probe ระดับแพลตฟอร์มเป็น overhead ของเรา **ไม่นับเข้าบิลของร้าน** — และห้าม
  เขียน UI/doc ให้ `actualCostUsd` ดูเหมือนใบแจ้งหนี้ของ provider (มันคือ cost attribution)
- **index ที่เพิ่ม**: `created_at DESC` (ตาราง recent usage) + partial index
  `(tenant_id, year_month, created_at) WHERE status='started' AND completed_at IS NULL` (ตัวกวาด stale)
- wire แล้วทั้ง chat, pharmacy, follow-up, report generation, BYOK test, slip OCR · regression
  ครอบ retry / usage บางส่วน / model ที่ไม่รู้จัก / rate card ของแต่ละ provider อยู่ใน
  [`scripts/ai-eval/`](../scripts/ai-eval/README.md) (`runtime-contract`, `slip-reader-contract`,
  `pharmacy-intake-contract`)
- **ยังไม่ทำ**: apply `7.82` เข้า production · ไม่มีการกระทบยอดกับใบแจ้งหนี้จริงของ provider (ไม่ได้
  ตั้งใจจะมี) · rate card ยังเป็นค่าคงที่ในโค้ด/env ไม่ได้ดึงราคาสดจาก provider
...
[docs/integrations/lazada.md](integrations/lazada.md)) ถ้าจะกลับไปทำ OAuth+Sync จริงในอนาคต ต้องรีวิว/เขียนแผนใหม่
...
โมดูลเชิงปฏิบัติการครบแล้ว (ดูตาราง Build Status ใน [docs/architecture/system.md](architecture/system.md)
และประวัติรายฟีเจอร์ใน [docs/feature-log.md](feature-log.md)) + **Customer 360 panel ใน Inbox เสร็จแล้ว** (ดูหัวข้อด้านบน)
+ **แท็บ "ลูกค้า"/merge/reorder เสร็จแล้ว** + **Shopee/Lazada beta scaffold เสร็จแล้ว** (ดู [docs/integrations/lazada.md](integrations/lazada.md))
...
carrier booking hardening ใน § Carrier scaffold ด้านบน + [docs/integrations/carriers.md](integrations/carriers.md)
