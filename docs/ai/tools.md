...
| --- | --- | --- | --- | --- |
| `search_system_capabilities`, `search_system_guides` | A1/knowledge | no | — | deterministic read of the verified bilingual catalog; `search_system_guides` also returns the verified FAQ answers owned by each guide, to be quoted rather than paraphrased; reports missing actor permissions but never grants access |
