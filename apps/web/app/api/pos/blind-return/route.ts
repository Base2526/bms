...
async function handlePOST(req: NextRequest) {
  ...
  if (!(await cashierHasPermission(device.tenantId, approver.userId, "pos.return.noreceipt"))) {
    return NextResponse.json({ error: await posPermissionDeniedMessage(device.tenantId, "pos.return.noreceipt", { secondPerson: true }) }, { status: 403 });
  }
...
export const POST = withRouteErrorLog("POST /api/pos/blind-return", handlePOST);
