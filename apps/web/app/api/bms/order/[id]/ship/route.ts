// POST /api/bms/order/:id/ship — OMS transition (shipOrder) [signed admin + RBAC · tenant จาก session]
...
async function handlePOST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const ok = await shipOrder(auth.tenantId, orderId);
  if (!ok) return NextResponse.json({ status: "INVALID_TRANSITION", orderId }, { status: 409 });
  return NextResponse.json({ ok: true, orderId });
}

export const POST = withRouteErrorLog("POST /api/bms/order/[id]/ship", handlePOST);
