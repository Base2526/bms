...
export default function Page() {
  ...
  const anchorItems = useMemo(
    () => [
      ...
      { key: "archetypes", href: "#archetypes", title: "ตัวอย่างตามประเภทร้าน" },
      ...
      <Row gutter={[20, 20]} align="top">
        <Col xs={24} lg={17}>
          <Space direction="vertical" size={18} style={{ width: "100%" }}>
            ...
            <Section
              id="archetypes"
              title="🛍 ตัวอย่างถาม-ตอบตาม businessArchetype"
              subtitle="ใช้ส่วนนี้ตอนอธิบายทีม, demo ระบบ, หรือเช็กว่า AI ของร้านควรตอบและพาลูกค้าไปสู่ flow หลังบ้านแบบไหน"
            >
              <Space direction="vertical" size={16} style={{ width: "100%" }}>
                <Alert
                  ...
                  showIcon
                  style={{ borderRadius: 14 }}
                  message="หลักคิดสำคัญ"
                  description="BMS ไม่ได้จบที่ตอบแชท แต่ต้องพาลูกค้าไปสู่การทำงานจริงของร้าน: รู้จักลูกค้า -> หา/แนะนำสินค้า -> เช็กสต็อก -> สร้างออเดอร์ -> รับชำระ -> ส่งของ -> ติดตามสถานะ และเมื่อของหมดควรเปลี่ยนบทสนทนาเป็น restock queue เพื่อเก็บโอกาสขายกลับมา"
                />

                <Row gutter={[14, 14]}>
                  {archetypeExamples.map((item) => (
                    <Col xs={24} key={item.key}>
                      <Card style={{ borderRadius: 16 }}>
                        <Space direction="vertical" size={10} style={{ width: "100%" }}>
                          ...
                          <div>
                            <Text strong>ตัวอย่างคำถามลูกค้า</Text>
                            <Paragraph style={{ margin: "6px 0 0" }}>{item.customerAsk}</Paragraph>
                          </div>
                          <div>
                            <Text strong>แนวตอบของ AI</Text>
                            <Paragraph type="secondary" style={{ margin: "6px 0 0" }}>
                              {item.aiReply}
                            </Paragraph>
                          </div>
                          <div>
                            <Text strong>flow หลังบ้านที่ควรเกิด</Text>
                            <Paragraph style={{ margin: "6px 0 0" }}>{item.backendFlow}</Paragraph>
                          </div>
                          <Text type="secondary">ทำไม archetype นี้สำคัญกับ BMS: {item.whyItFits}</Text>
                        </Space>
                      </Card>
                    </Col>
                  ))}
