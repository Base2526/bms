...
> = {
  products: {
    ...
    tags: ["Products", "นำเข้า CSV/XLSX", "Gallery", "Stock", "Category"],
  },
  orders: {
    ...
    checks: [
      ...
      "กด คูปอง เพื่อเลือกโค้ดที่เปิดใช้งาน ระบบจะใส่ข้อความคูปองใน draft และเมื่อกดส่งจริงจะแนบลิงก์ Claim coupon ให้ลูกค้ากดใช้",
...
const couponConditions = [
  { code: "SAVE10", condition: "ลด 10%, active, ยังไม่หมดอายุ, quota เหลือ, ลูกค้ายังไม่เกิน per-customer limit, ยอดถึงขั้นต่ำ", result: "ผ่าน: ระบบแนบ claim link ให้ลูกค้ากดใช้ได้ และตอนสร้างออเดอร์ backend จะลดราคาจริงใน transaction เดียวกับการจองสต็อก" },
  ...
  { code: "SAVE20", condition: "ลูกค้ากด claim link แล้วสร้างออเดอร์ แต่ยังไม่จ่ายเงิน", result: "สถานะเป็น RESERVED และผูก order id ไว้: ถ้า order ถูก cancel หรือหมดเวลาจ่าย ระบบคืน quota และย้อนกลับเป็น CLAIMED/ASSIGNED ตามประวัติเดิม" },
...
const couponWalletStates = [
  ...
  { state: "CLAIMED", meaning: "ลูกค้ากด claim link/button เพื่อยืนยันว่าจะใช้โค้ดนี้แล้ว แต่ยังไม่ได้สร้างออเดอร์สำเร็จ" },
                    ...
                    <Steps
                      ...
                      items={[
                        ...
                        },
                        {
                          ...
                          description: "กดปุ่ม คูปอง ที่ composer แล้วเลือกโค้ด ระบบจะแทรกข้อความ fallback ให้ เช่น โค้ด, ส่วนลด, ขั้นต่ำ, วันหมดอายุ, สิทธิ์ที่เหลือ เมื่อกดส่งจริง ระบบจะบันทึกสิทธิ์นี้ไว้ใน customer coupon wallet และแนบ claim link ที่ลูกค้ากดใช้ได้ทันที",
                        },
                        {
                          title: "3. ลูกค้าถามว่ามีคูปองอะไร หรือพิมพ์โค้ดมา",
                          description: "AI เรียก list_customer_coupons ก่อนถ้าลูกค้าถามคูปองของตัวเองหรือถามว่าอะไรใกล้หมดอายุ และใช้ list_available_coupons / check_coupon ตามบริบท ห้ามเดาเองว่าคูปองใช้ได้ ถ้าลูกค้าพิมพ์โค้ด ระบบจะตอบสถานะ/เงื่อนไขเท่านั้น การ mark เป็น CLAIMED ต้องมาจาก claim link/button",
                        },
                        {
