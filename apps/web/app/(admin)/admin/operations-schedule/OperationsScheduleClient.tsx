  ...
  return (
    <div>
            ...
            <div>
              ...
              {runsByJob[row.key]?.length ? (
                ...
                <Paragraph type="secondary" style={{ margin: "4px 0 0" }}>
                  No recorded runs yet — either this job hasn't fired since 7.53__bms_job_runs.sql was applied, or (for release-expired/channel-health/ai-health/report-digest) no external scheduler is calling it yet.
                </Paragraph>
              )}
            </div>
            {row.aiView ? (
      ...
      <Alert type="info" showIcon style={{ marginTop: 16 }} icon={<InfoCircleOutlined />}
        message="Requires migration 7.53" description="Run history is read from bms_job_runs (db/migrations/7.53__bms_job_runs.sql). Until that migration is applied, every job on this page will show 'Never run yet' even if its cron endpoint is being called." />
    </div>
  );
}
