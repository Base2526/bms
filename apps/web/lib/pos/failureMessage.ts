...
/**
 * ความล้มเหลว "ระดับการเชื่อมต่อ" ที่ยังไม่ถึง service — reverse proxy ตอบ 502/503/504
 * หรือเน็ตร้านหลุด · ต่างจาก describePosFailure() ที่แปลสถานะซึ่ง server ตั้งใจตอบมา
 *
 * เคสจริงที่เจอ: Caddy ตอบ 502 เป็น **HTML** ไม่ใช่ JSON → `response.json()` พัง →
 * body ว่าง → ไม่มีทั้ง `status` และ `error` → หน้าจอโชว์ "HTTP 502" ดิบ ๆ ซึ่งเป็น
 * ปัญหาเดียวกับที่เคยแก้ให้ 409 ไปแล้ว (พนักงานอ่านแล้วทำอะไรต่อไม่ได้)
 *
 * **ห้ามบอกว่า "ยังไม่ถูกบันทึก"** สำหรับ 502/504 — คำขออาจถึง server และ commit ไปแล้ว
 * ก่อน proxy จะหมดเวลารอ การชวนให้กดซ้ำทันทีจึงเสี่ยงทำงานซ้ำ (เช่น ส่งครัวสองรอบ)
 * คำตอบที่ถูกคือ "ยังไม่รู้ผล ให้รีเฟรชดูสถานะจริงก่อน"
 */
export function describeTransportFailure(status: number, online = true): string {
  if (!online) return "เน็ตหลุด — รายการยังไม่ถูกส่ง ต่อเน็ตแล้วกดอีกครั้ง";
  switch (status) {
    case 502:
    case 503:
    case 504:
      return `เซิร์ฟเวอร์ไม่ตอบ (${status}) — ยังไม่รู้ว่ารายการถูกบันทึกหรือไม่ กดรีเฟรชเพื่อดูสถานะจริงก่อน อย่ากดซ้ำทันที ถ้ายังไม่หายให้แจ้งผู้ดูแลระบบ`;
    case 401:
      return "เครื่องนี้ไม่ได้จับคู่แล้ว — จับคู่เครื่อง POS ใหม่จากหลังบ้าน";
    case 403:
      return "บัญชีนี้ไม่มีสิทธิ์ทำรายการนี้ — ให้คนที่มีสิทธิ์กด PIN";
    case 404:
      return "ไม่พบเส้นทางนี้บนเซิร์ฟเวอร์ — เวอร์ชันหน้าจอกับเซิร์ฟเวอร์อาจไม่ตรง ให้โหลดหน้าใหม่";
    case 429:
      return "ยิงคำขอถี่เกินไป — รอสักครู่แล้วลองอีกครั้ง";
    default:
      return status >= 500
        ? `เซิร์ฟเวอร์ผิดพลาด (${status}) — ยังไม่รู้ว่ารายการถูกบันทึกหรือไม่ กดรีเฟรชเพื่อดูสถานะจริงก่อนลองใหม่`
        : `ทำรายการไม่สำเร็จ (${status}) — โหลดหน้าใหม่แล้วลองอีกครั้ง`;
  }
}
