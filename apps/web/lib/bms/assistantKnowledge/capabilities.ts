...
export const SYSTEM_CAPABILITIES: readonly SystemCapability[] = [
  ...
  },
  {
    ...
    aliases: aliases(["รวมแชททุกช่องทาง", "ทุกช่องทางที่เดียว", "omnichannel", "อินบ็อกซ์", "แชทลูกค้า", "ข้อความ"], ["all chat channels in one place", "omnichannel inbox", "inbox", "customer chat", "messages"]),
  ...
  {
    id: "inventory.stock-model", module: "inventory",
    title: both("รูปแบบสต็อกตามประเภทร้าน", "Per-shop stock models"),
    ...
    limitations: both("บรรทัดที่ขายไปแล้วถือ snapshot ของส่วนประกอบที่ตัดจริง แก้สูตรทีหลังจึงไม่ย้อนไปเปลี่ยนบิลเดิม และสต็อกยังถือเป็นจำนวนเต็มหน่วยฐานเสมอ", "A sold line keeps the snapshot of what it consumed, so editing a recipe never rewrites an existing bill, and stock is still held in whole base units."),
  },
  {
    id: "inventory.wastage-ledger", module: "inventory",
    title: both("บันทึกของเสีย", "Wastage ledger"),
    description: both("ตัดของที่ขายต่อไม่ได้ออกจากสต็อกพร้อม movement และ audit ทุกครั้ง", "Write off unsellable stock with a stock movement and an audit entry every time."),
    aliases: aliases(["ของเสีย", "ตัดของเสีย", "ของหมดอายุ", "ของแตกหัก"], ["wastage", "write off", "expired stock", "damaged stock"]),
    status: "AVAILABLE", route: "/admin/wastage", requiredPermissions: ["stock.adjust"],
    configurationDependencies: [],
    limitations: both("ตัดแล้วย้อนกลับเองไม่ได้ ต้องปรับสต็อกกลับเป็นอีกรายการ", "A write-off cannot undo itself; reversing it takes a separate stock adjustment."),
  },
  {
    id: "kitchen.workflow", module: "kitchen",
    title: both("คิวครัวตาม station", "Kitchen workflow by station"),
    description: both("เมนูที่ขายถูกส่งเข้าคิวครัวตาม station แล้วเดินสถานะจากรายการใหม่จนเสิร์ฟ", "Sold menu lines enter the kitchen queue by station and move from new through to served."),
    aliases: aliases(["คิวครัว", "กระดานครัว", "ร้านอาหาร", "station", "ระบบรองรับครัวไหม"], ["kitchen queue", "kitchen board", "restaurant", "station", "does it support a kitchen"]),
    status: "CONDITIONAL", route: "/admin/kitchen", requiredPermissions: ["order.view"],
    configurationDependencies: ["The kitchen workflow capability must be on and products must carry a station."],
    limitations: both("กระดานเดินสถานะอย่างเดียว ไม่ขยับสต็อกและไม่ยกเลิกบิล และยังไม่มีจอครัวแยกสำหรับพนักงานที่ไม่มีสิทธิ์ดูออร์เดอร์", "The board only moves status: it never moves stock or cancels a bill, and there is no separate kitchen display for staff without order access."),
  },
  {
  ...
  },
  {
    ...
    aliases: aliases(["ตามลูกค้าที่หายไป", "ลูกค้าไม่กลับมา", "ดึงลูกค้ากลับ", "ติดตามลูกค้า", "follow up", "retention", "ลูกค้าเงียบ"], ["win back customers", "lapsed customers", "follow-up", "retention", "quiet customers"]),
  ...
  },
  {
    ...
    aliases: aliases(["แจ้งลูกค้าเมื่อของเข้า", "ของเข้าแล้วแจ้ง", "แจ้งเตือนสินค้าเข้า", "ลูกค้ารอของ", "restock"], ["notify when back in stock", "restock alert", "waiting customers", "restock"]),
  ...
  },
  {
    ...
    aliases: aliases(["ขายยกกล่อง", "ขายเป็นแพ็ก", "ยกลัง", "แพ็กสินค้า", "ขายเป็นกล่อง", "ขายเป็นแผง", "pack code"], ["sell by the box", "case pack", "product pack", "sell by box", "selling unit", "pack code"]),
  ...
  },
  {
    ...
    aliases: aliases(["เชื่อมช่องทางแชท", "ช่องทางที่รองรับ", "ตั้งค่าร้าน", "เชื่อม LINE", "บัญชีรับเงิน", "ตั้งค่า AI"], ["supported chat channels", "connect a channel", "store settings", "connect LINE", "payment account", "AI settings"]),
  ...
  },
  {
    ...
    aliases: aliases(["โควตาการใช้ AI", "ใช้ AI ได้เท่าไร", "แพ็กเกจ", "โควตา AI", "เครดิต", "billing"], ["usage allowance", "plan", "AI quota", "credits", "billing"]),
  ...
  },
  {
    ...
    aliases: aliases(["ตั้งเวลาส่งรายงานอัตโนมัติ", "รายงานส่งเองตามเวลา", "ตั้งเวลารายงาน", "ส่งรายงานอัตโนมัติ", "report schedule"], ["schedule an automatic report", "schedule report", "automatic report", "report schedule"]),
  ...
  featureCapability({
    ...
    aliases: [["โอนสต็อกข้ามสาขา", "ย้ายของข้ามสาขา", "โอนของ", "ย้ายสต็อก", "ของระหว่างทาง"], ["move stock between branches", "stock transfer", "move stock", "in transit"]], requiredPermissions: ["inventory.transfer"],
  ...
  featureCapability({
    ...
    aliases: [["ตรวจนับสต็อก", "รองรับการตรวจนับสต็อก", "นับสต็อก", "ตรวจนับ", "stock count"], ["support stock counting", "stock count", "cycle count", "inventory count"]], requiredPermissions: ["inventory.count"],
  ...
  featureCapability({
    ...
    aliases: [["เครดิตร้านหรือบัตรของขวัญ", "บัตรของขวัญ", "เครดิตร้าน", "เงินคงเหลือลูกค้า", "store credit"], ["gift card or store credit", "store credit", "customer credit balance"]], status: "CONDITIONAL", anyOfPermissions: ["storecredit.issue", "storecredit.redeem", "storecredit.adjust"],
  ...
  featureCapability({
    ...
    aliases: [["คืนสินค้าหน้าร้าน", "รับคืนที่เคาน์เตอร์", "คืนสินค้า", "void", "คืนไม่มีบิล"], ["counter returns", "return", "void", "no receipt return"]], status: "CONDITIONAL", anyOfPermissions: ["order.return", "pos.void", "pos.return.noreceipt"],
  ...
  featureCapability({
    ...
    aliases: [["ต่อเครื่องพิมพ์สลิปกับลิ้นชัก", "ปริ้นใบเสร็จ", "เครื่องพิมพ์สลิป", "เปิดลิ้นชัก", "ESC/POS"], ["receipt printer and cash drawer", "receipt printer", "print receipt", "cash drawer", "ESC/POS"]],
