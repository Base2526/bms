...
function tokens(value: string): string[] {
  return normalizeAssistantQuery(value).split(" ").filter((token) => token.length > 1);
}
...
function guideResult(
  ...
  const queryScore = textScore(query, entry.title[locale], entry.aliases[locale], `${entry.summary[locale]} ${entry.steps[locale].join(" ")}`);
  let score = queryScore;
