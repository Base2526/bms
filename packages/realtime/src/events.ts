...
export const REALTIME_EVENT_TYPES = [
  ...
  "pos.deposit.changed",
  "pos.expense.changed",
  "pos.no_sale.recorded",
  "pos.parked_sale.changed",
  "pos.petty_cash.changed",
  ...
  "pharmacy.authorization.changed",
  "order.tax_document.changed",
  "order.loyalty.changed",
...
export const REALTIME_EVENT_RULES: Readonly<Record<RealtimeEventType, RealtimeEventRule>> = {
  ...
  "pos.cash_movement.recorded": location(["pos.shift.report"]),
  "pos.return.changed": location(["order.view"]),
  "pos.deposit.changed": location(["order.view"]),
  "pos.expense.changed": location(["pos.shift.report"]),
  "pos.no_sale.recorded": location(["pos.shift.report"]),
  "pos.parked_sale.changed": location(["order.view"]),
  "pos.petty_cash.changed": location(["pos.shift.report"]),
  ...
  "kitchen.ticket.created": location(["order.view"]),
  "kitchen.ticket.status_changed": location(["order.view"]),
  ...
  "board_game.waitlist.changed": location(["board_game.session.manage"]),
  ...
  "payment.store_credit.changed": tenant(["payment.view"]),
  "payment.ar.changed": tenant(["payment.view"]),
  ...
  "purchase.order.changed": tenant(["purchase.view"]),
  "inventory.wastage.recorded": location(["product.view"]),
  ...
  "pharmacy.authorization.changed": location(["order.view"]),
  "order.tax_document.changed": location(["order.view"]),
  "order.loyalty.changed": tenant(["order.view"]),
...
const DOMAIN_FLAG: Readonly<Record<string, string>> = {
  ...
  // แยกธงกันจะได้กระดานที่สดครึ่งเดียว ซึ่งแย่กว่าเลือกทางใดทางหนึ่ง
  kitchen: "REALTIME_RESTAURANT_ENABLED",
